...
- **Single Pane Width** — a tab with no splits is capped at this width and centered, whatever it is showing: a terminal, a browser page, an editor or a preview. Diffs and conflict review always use the full window. Drag a lone terminal's edges to change it. Splitting the tab uses the full width again. Set it to `0` to always use the full width.

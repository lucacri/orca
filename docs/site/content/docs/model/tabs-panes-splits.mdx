...
A tab area showing exactly one pane is capped at the **Single Pane Width** (Settings → Appearance) and centered. While it shows one pane, the next pane that opens — a file, a diff, a browser tab, a new terminal, or a tab an agent opens — opens as a right-hand split beside it rather than as a tab on top of it, and the first pane then fills its half. The exception is a browser tab opening while that pane already shows a browser: it joins as another tab. This happens at any window width, even with the cap turned off, and on remote workspaces as well as local ones. With two panes showing, new tabs open in the focused pane as usual; explicit split commands still let you build larger layouts. Reopening a closed tab puts it back where it was, and restoring a saved session — or mirroring a layout from a remote runtime — keeps the arrangement it already had.

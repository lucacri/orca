...
A tab area showing exactly one pane is capped at the **Single Pane Width** (Settings → Appearance) and centered. When a lone terminal is actually being narrowed by that cap, clicking a file path in it — or a file link in its agent chat — opens the file as a right-hand split instead of a tab in the same group, and the terminal then fills its half. The result is an ordinary split with ordinary split controls.
